// let node = document.getElementById('im1').cloneNode(true);
...
<button id="im${n}Button" onclick="imClick(${n})" class="tab-button width-40">0</button>
</div>`;

const div = document.getElementById('ims');
for (var i = 2; i <= 10; i++) {
    div.insertAdjacentHTML('beforeend', base(i));
}

for (var i = 1; i <= 10; i++) {
  imLevels.push(new ImLevel(i, new Decimal(0), new Decimal(0), new Decimal(1), false, false));
  setText(`im${i}Button`, imLevels[i - 1].cost());
}
